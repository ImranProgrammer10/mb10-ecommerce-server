...
const uri = `mongodb+srv://${process.env.USER_NAME}:${process.env.USER_PASS}@cluster0.bymuqus.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
    ...
    try {
        ...
        const shippingCollection = client.db("productList").collection("shipping");
        ...
        app.post("/shipping",async(req,res)=>{
            const result=await shippingCollection.insertOne(req.body);
            res.send(result);
        });

        app.get('/shipping', async (req, res) => {
            const query = {};
            const cursor = await shippingCollection.find(query).toArray();
            res.send(cursor)
        })
